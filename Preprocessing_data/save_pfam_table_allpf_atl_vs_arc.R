...
n <- 792
if (taxo != "0" ){
  file <- readRDS(paste('MetaTG_pfams_station_table_allpf_',fraction,'_atl_vs_arc',id,'_',tax_id,'.rds', sep=''))
} else{
  file <- readRDS(paste('MetaTG_pfams_station_table_allpf_',fraction,'_atl_vs_arc',id,'.rds', sep=''))
}
...
if (n_clusts %in% c(2,5, 6) & taxo != "0"){
  types <- paste(c('T_com', 'uni_T_com', 'G_com','uni_G_com' ,
             'T_atl', 'uni_T_atl', 'G_atl','uni_G_atl',
             'T_arc', 'uni_T_arc', 'G_arc','uni_G_arc'), tax_id, sep='_')
  ...
  types <- c('T_com', 'uni_T_com', 'G_com','uni_G_com' ,
             'T_atl', 'uni_T_atl', 'G_atl','uni_G_atl',
             'T_arc', 'uni_T_arc', 'G_arc','uni_G_arc')
} else if (n_clusts %in% c(3,4) & taxo == "0"){
...
for (j in seq0){
  ...
  if (taxo =="0"){
    ...
    saveRDS(table_pfam, paste('pfams_station_table_',types[j],'_allpf_',fraction,id,'.rds', sep=''))
  } else{
    for (pfam in pfams_all){
      ...
      if (!is.null(data)){
        ...
        list_pfam_taxo[[pfam]] <- data[,-1]
      }
    }
    saveRDS(list_pfam_taxo, paste('pfams_station_table_',types[j],'_allpf_',fraction,id,'.rds', sep=''))  
  }
}
