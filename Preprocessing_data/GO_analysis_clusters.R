...
env=env[env$Station %in% c(142, 144, 145,  147, 148 ,150, 151, 152,155, 158,163, 168 ,173, 175, 178,180, 188, 189,193, 194, 196),]
#tests=c('hommel', 'fdr')
  ...
  for (type in types){
    ...
    outliers_functions <- function(GO_data, gr){
      ...
      for (nam in names(GO_data)){
        t0 <- GO_data[[nam]]$all
        if (!is.null(gr)){
           if (is.null(dim(t0)[1]) & !is.null(t0)){
	      nas=names(t0)
              #print(t0)
              t0=data.frame(t(t0))
              colnames(t0)=nas
	  ...
	  t0 <- t0[,!(colnames(t0) %in% c('142SUR','201SUR','205SUR',
          ...
          if (is.null(d_t0)){
            t1 <- t0#t(t0)
            sum_metaT <- as.numeric(t1[1,])
            #t0 = data.frame(t(t0))
            #print(dim(t0))
	    #colnames(t0)= c(atlantic_stations[!(atlantic_stations %in% c('142SUR','201SUR','205SUR',
            #                              '206SUR', '208SUR', '209SUR', '210SUR'))],arctic_stations[!(arctic_stations %in% c('142SUR','201SUR','205SUR',
            #                              '206SUR', '208SUR', '209SUR', '210SUR'))])
          } else{
          ...
          if (sum(sum_metaT==0)==0){
            #print(length(c(log(sum_metaT[cond_atl]),log(sum_metaT[cond_arc]) )))  
            cond_atl <- colnames(t0) %in% atlantic_stations
            cond_arc <- colnames(t0) %in% arctic_stations
	    #print(length(c(log(sum_metaT[cond_atl]),log(sum_metaT[cond_arc]) )))
	    #print(log(sum_metaT[cond_atl]))
            #print(log(sum_metaT[cond_arc]))
            w_test <- t.test(log(sum_metaT[cond_atl]), log(sum_metaT[cond_arc]))$p.value
            
            #print(sum_metaT[cond_atl])
            #print(mean(sum_metaT[cond_arc]))
	    ...
	    #print(length(env$Month))
            #print(c(log(sum_metaT[cond_atl]),log(sum_metaT[cond_arc]) ))
            #print(sum_metaT)
            #print(cond_atl)
            dfi=data.frame(Basin=c(rep('NAO', 8), rep('AO',12 )), y=c(log(sum_metaT[cond_atl]),log(sum_metaT[cond_arc]) ), Month=env$Month)
	    model <- lm(y ~ Basin + Basin:Month, data = dfi)
	    a=anova(model)
	    p_month <- a["Basin:Month", "Pr(>F)"]
            p_vals_month=c(p_vals_month, p_month)
          } else{
            GOs_all <- append(GOs_all, nam)
          }
        }
      ...
      test_holm_month=p.adjust(p_vals_month, method = 'hommel')
      ...
      arc_month=GO_table[GO_table[,1] %in% GOs[test_holm<0.05 & loc=='arctic' & test_holm_month<0.05] ,1]
      atl_month=GO_table[GO_table[,1] %in% GOs[test_holm<0.05 & loc=='atlantic' & test_holm_month<0.05] ,1]
      ...
      if (is.null(gr)){
        ...
        saveRDS(arc_month, paste('GO_representative_',type,'_arctic_', fraction,'_' ,taxo,bis, '_month.rds', sep=''))
        saveRDS(atl_month, paste('GO_representative_',type,'_atlantic_', fraction, '_' ,taxo, bis,'_month.rds', sep=''))
      } else{
        ...
        saveRDS(arc_month, paste('GO_representative_',type,'_arctic_', fraction,'_' ,taxo,'_', gr,bis,'_month.rds', sep=''))
        saveRDS(atl_month, paste('GO_representative_',type,'_atlantic_', fraction, '_' ,taxo,'_', gr,bis, '_month.rds', sep=''))
      }
      ...
      arc_month=GO_table[GO_table[,1] %in% GOs[test_fdr<0.05 & loc=='arctic' & test_holm_month<0.05] ,1]
      atl_month=GO_table[GO_table[,1] %in% GOs[test_fdr<0.05 & loc=='atlantic' & test_holm_month<0.05] ,1]
      ...
      if (is.null(gr)){
        ...
        saveRDS(arc_month, paste('GO_representative_',type,'_arctic_', fraction,'_' ,taxo,bis, '_fdr_month.rds', sep=''))
        saveRDS(atl_month, paste('GO_representative_',type,'_atlantic_', fraction, '_' ,taxo, bis,'_fdr_month.rds', sep=''))
      }
