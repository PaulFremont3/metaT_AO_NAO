...
extract_pfam_metaT <- function(arg){
  ...
  output_list <- rep(list(NA), (n_clusts0+1)*6)
  for (cl in 1:(n_clusts0+1)){
    ...
    if (length(ok_uid)>0){
      ...
      output_list[[(cl-1)*6+1]]=data_pf1
      output_list[[(cl-1)*6+2]]=data_pf2
      output_list[[(cl-1)*6+3]]=data_pf3
      output_list[[(cl-1)*6+4]]=data_pf4
      output_list[[(cl-1)*6+5]]=data_pf5
      output_list[[(cl-1)*6+6]]=data_pf6
    }
  }
...
if (taxo==''){
  saveRDS(whole_data, paste('MetaTG_pfams_station_table_allpf_',fraction,'_atl_vs_arc',id,'_taxo_bis.rds', sep=''))
} else{
  saveRDS(whole_data, paste('MetaTG_pfams_station_table_allpf_',fraction,'_atl_vs_arc',id,'_taxo_',taxo,'_bis.rds', sep=''))
}
