...
barplot_tax <- function(components, name, frac, subset){
  ...
  mtx=min(c(length(rownames(test)), M))
  print('here')
  if (name=='physical_clr'){
    to_check=c(rownames(test)[1:mtx], 'MBPP')
  } else{
    to_check=rownames(test)[1:mtx]
  }
  ...
  for (tax in to_check){
    ...
    if (tax!='MBPP'){
      ...
      test1 <- test0[test0$V1 %in% subts,]
      test1 <- test1[test1$V2=='T' & test1$V3=='-',]
      if (dim(test1)[1]>2){
        tab <- table(test1$V4, test1$V1)
        go_shared <- rownames(tab)[rowSums(tab > 0) == ncol(tab)]
        test1 <- test1[test1$V4 %in% go_shared, ]
      }
      #print(dim(test1))
      if (dim(test1)[1]>2){
        test1$V1='MBPP'
      }
      if (name=='physical_clr' & frac=='GGZZ'){
         print('physical_clr')
         print(frac)
         print(dim(test1))
         saveRDS(test1, paste('test1_T_clr.rds'))
         saveRDS(tab, 'tab_T_clr.rds')
      }
    }
    if (dim(test1)[1]>2){
    ...
    if (dim(test2)[1]>1){
      ...
      if (tax!='MBPP'){
        ...
        baseline_matou_tax <- baseline_matou[baseline_matou$Group.1 %in% subts, c(2,3)]
        print('h1')
        baseline_matou_tax <- aggregate(x ~ Group.2, data = baseline_matou_tax, sum)
      }
      print('h2')
      ...
      for (i in 1:dim(test2)[2]){
        ...
        if (tax=='MBPP'){
          p_values_adj <- p.adjust(p_values, method = 'hommel')
          print(frac)
	  print('p_values_adj')
          print(p_values_adj)
        } else{
	...
	co <- 1
        if (is.null(dim(test3)) & length(test3)>0 & tax=='MBPP'){
