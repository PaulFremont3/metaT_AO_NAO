library('sm')
vioplot1 <- function(x,range=1.5,h=NULL,ylim=NULL,names=NULL, horizontal=FALSE,title=NULL,las=2,
                    col="magenta", border="black", lty=1, lwd=1, rectCol="black", colMed="white", pchMed=19, at, add=FALSE, wex=1,
                    drawRect=TRUE, ylab=NULL, cex=1)
{
  ...
  if(!horizontal) {
    if(!add){
      plot.window(xlim = xlim, ylim = ylim, main=title)
      axis(2, cex.axis=cex)
      axis(1,at = at, labels=label ,las=las,cex.axis=cex, tick = F)
    }
    mxy <- NULL
    mxx <- NULL
    # box()
    for(i in 1:n) {
      ...
      # %% length(col) + 1
      mxx <- append(mxx, max(c(base[[i]], rev(base[[i]]))) )
      mxy <- append(mxy, max(c(at[i]-height[[i]], rev(at[i]+height[[i]])) ))      
      if(drawRect){
        # plot IQR
    ...
    #print(max(mxx, na.rm=T)/2)
    #print(max(mxy/2, na.rm=T))
    mtext(title,side=3,outer=TRUE,padj=3)
    #text(n/2+1, max(mxy, na.rm=T)/2+5 , title)    
  }
  else {
    if(!add){
      plot.window(xlim = ylim, ylim = xlim)
      axis(1)
      axis(2,las=las,at = at, label=label )
    }
    ...
    for(i in 1:n) {
      ...
      mxy <- append(mxy, max(c(base[[i]], rev(base[[i]])) ) )
      mxx <- append(mxx, max(c(at[i]-height[[i]], rev(at[i]+height[[i]])) ) )
      if(drawRect){
        # plot IQR
