...
barplot_GO <- function(GO_list, fraction, taxo, n_clusts, types, name_list, group=NULL, bis){
  ...
  for (go in names(GO_data1)){
    ...
    if (!is.null(GO_data1[[go]][[1]])){
      ...
      rcl=clean_rownames(rownames(t0))
      rownames(t0)=rcl
      ...
      other_taxo <- c( 'root','Viruses', 'other Viridiplantae',
      ...
      if (sum(sum_metaT==0)==0){
        ...
        vioplot(log(sum_metaT[cond_atl]), log(sum_metaT[cond_arc]), names=c('Atlantic', 'Arctic'),
                col=c('darkorange', 'darkblue'), cex=2.5, cex.axis=2)
...
tys <- c('T', 'G')
taxo = 'taxo_groups3'#commandArgs(trailingOnly = T)[1]
n_clusts <- '6'#commandArgs(trailingOnly = T)[2]
bis <- '_bis'#commandArgs(trailingOnly = T)[3]
fractions <-c('GGZZ')
...
clean_rownames=function(h){
  tax_clean <- sub("\\.+$", "", h)
  
  # Replace remaining periods with spaces
  tax_clean <- gsub("\\.", " ", tax_clean)
  
  # Replace multiple spaces with a single space
  tax_clean <- gsub("\\s+", " ", tax_clean)
  return(tax_clean)
}
