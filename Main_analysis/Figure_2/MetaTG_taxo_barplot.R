...
# list_to_plot <- list()
# for (clus in clusts){
#   data0 <- readRDS(paste('Meta', clusts[c],'_',tax_id,'_', frac,'.rds', sep=''))
#   list_to_plot[[c]]<-data0
#   c=c+1
# }
...
if (type=='G' & subset == '0'){
  ...
  #colors_taxon$col <- as.character(levels(colors_taxon$col))[colors_taxon$col]
  #colors_taxon$taxon <- as.character(levels(colors_taxon$taxon))[colors_taxon$taxon]
...
if (subset=='0'){
  ...
  if (type %in% c('G' , 'uni_G')){
    ...
    #data0 <- data0[c(1:(d_ltp-(os+1)), dim(data0)[1]),]
    data0=data0[anti_sel,]
    data0 <- rbind(data0, dt)
  } else{
    #print(mean(apply(data0, 2, sum)))
    root_row=which(rownames(data0)=='root')
    sel=unique(c((d_ltp-os):d_ltp, root_row))  
    dt <- apply(data0[sel,], 2, sum)
    anti_sel=1:d_ltp
    anti_sel=anti_sel[!(anti_sel %in% sel)]
    #data0 <- data0[1:(d_ltp-(os+1)),]
    data0=data0[anti_sel,]
    data0 <- rbind(data0, dt)
    #print(mean(apply(data0, 2, sum)))
  }
...
#sts <- colnames(data0)
#sts <- sapply(sts, function(x){str_replace(x, 'SRF', 'SRF')})
#transition <- sts[sts %in% c('155SRF' , '158SRF', '163SRF')]
transition <- c('')
...
#     if (type %in% c('G' , 'uni_G')){
#       d_ltp <- d_ltp-1
#       dt <- apply(data0[(d_ltp-os):d_ltp,], 2, sum)
#       data0 <- data0[c(1:(d_ltp-(os+1)),dim(data0)[1] ),]
#       data0 <- rbind(data0, dt)
#     } else{
#       dt <- apply(data0[(d_ltp-os):d_ltp,], 2, sum)
...
#par(mfrow=c(1,2))
#barplot(as.matrix(data0[2:dim(data0)[1],]), las=2,
#        col = as.character(colors_taxon$col[match(rownames(data0)[2:dim(data0)[1]], colors_taxon$taxon)]))
#plot(0,0, col='white', xaxt = 'n', yaxt='n', xlab = '', ylab = '', axes=F)
#legend('topleft',legend = rownames(data0)[2:51], 
#       fill=as.character(colors_taxon$col[match(rownames(data0), colors_taxon$taxon)])[2:51], 
#       box.lty=0 , ncol=2, cex=1)
#par(mfrow=c(1,2))
#barplot(as.matrix(data0[2:21,]), las=2,
#        col = as.character(colors_taxon$col[match(rownames(data0)[2:dim(data0)[1]], colors_taxon$taxon)]))
#plot(0,0, col='white', xaxt = 'n', yaxt='n', xlab = '', ylab = '', axes=F)
#legend('topleft',legend = rownames(data0)[2:21], 
#       fill=as.character(colors_taxon$col[match(rownames(data0), colors_taxon$taxon)])[2:21], 
#       box.lty=0 , ncol=2, cex=1)
...
if (type %in% c('uni_T', 'uni_G')){
  ...
  for (j in 2:4){
    ...
    n_tax <- dim(data0)[1]
    n_to_plot <- min(50,n_tax)
    #mxu <- max(apply(data0, 2, sum), na.rm=T)
    #print(mxu)
    #limy <- max(c(1, mxu))
...
#  data0 <- list_to_plot[[j]]
#  data0 <- data0[,!(colnames(data0) %in% c('142SRF','201SRF','205SRF', '206SRF', '208SRF', '209SRF', '210SRF'))]
#  data0 <- data0[2:dim(data0)[1],]
#  n_tax <- dim(data0)[1]
#  n_to_plot <- min(50,n_tax)
#  barplot(as.matrix(data0), las=2,
#          col = as.character(colors_taxon$col[match(rownames(data0), colors_taxon$taxon)]))
#  leg <-append(leg, rownames(data0))
#}
#leg <- unique(leg)
