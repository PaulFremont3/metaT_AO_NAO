...
for (frac in fracs){
  ...
  env=env[!(env$Station %in% c(143, 149, 146, 191)), ]
  env$Station <- paste(env$Station, 'SUR', sep='')
  colnames(mean_r)=env$Station
  rownames(mean_r)=env$Station
  ...
  cols <- colorRampPalette(c('red', 'white', 'blue'))(11)
